...
app.get("/ytdip", async (req, res) => {
    ...
    try {
        ...
        if (format === "mp3") {
            const response = await axios.get(`https://www.noobs-api.000.pe/dipto/ytDl3?link=${link}&format=mp3`);
            dat = response.data;
            p = `audio_${link}.mp3`;
        } else if (format === "mp4") {
            const response = await axios.get(`https://www.noobs-api.000.pe/dipto/ytDl3?link=${link}&format=mp4`);
            dat = response.data;
            p = `video_${link}.mp4`;
        } else {
